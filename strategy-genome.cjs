...
function saveGenome(genome) {
  ...
  fs.writeFileSync(tmpFile, JSON.stringify(genome, null, 2), 'utf8');
...
/**
 * Mutates a genome based on fitness score using noisy gradient descent.
 * Returns a NEW genome (deep copy). Original is NOT modified.
 *
 * @param {object} genome - Current genome
 * @param {number} fitnessScore - Normalized fitness [0,1]
 * @param {object} [options]
 * @param {number} [options.lr] - Learning rate override
 * @param {function} [options.noiseFn] - Noise function for testing
 * @param {boolean} [options.hasHumanRating] - Whether the fitness comes from human rating
...
function mutateGenome(genome, fitnessScore, options = {}) {
  ...
  if (newGenome.fitnessEMA === null) {
    newGenome.fitnessEMA = fitnessScore;
  } else {
    newGenome.fitnessEMA = EMA_ALPHA * fitnessScore + (1 - EMA_ALPHA) * newGenome.fitnessEMA;
  }

  // Gradient: positive if fitness > 0.5 (good), negative if < 0.5 (bad)
  const gradient = fitnessScore - 0.5;

  // Effective learning rate: higher for human ratings (more trusted signal)
  const effectiveLr = hasHumanRating ? lr * 1.5 : lr;

  // Mutate each gene
  for (const name of GENE_NAMES) {
    const noise = noiseFn ? noiseFn() : (Math.random() - 0.5) * NOISE_SCALE * 2;
    const delta = gradient * effectiveLr + noise;
